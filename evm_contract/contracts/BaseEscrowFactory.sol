...
import {ResolverValidationExtension} from "@1inch/limit-order-settlement/contracts/extensions/ResolverValidationExtension.sol";
 ...
 */
abstract contract BaseEscrowFactory is
    IEscrowFactory,
    ...
    function _postInteraction(
        ...
        if (MakerTraitsLib.allowMultipleFills(order.makerTraits)) {
            ...
            bytes32 key = keccak256(
                abi.encodePacked(
                    orderHash,
                    uint240(uint256(extraDataArgs.hashlockInfo))
                )
            );
        ...
        DstImmutablesComplement
            memory immutablesComplement = DstImmutablesComplement({
                maker: order.receiver.get() == address(0)
                    ? order.maker
                    : order.receiver,
                amount: takingAmount,
