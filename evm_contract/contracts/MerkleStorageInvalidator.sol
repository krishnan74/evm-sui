...
import {ExtensionLib} from "@1inch/limit-order-protocol-contract/contracts/libraries/ExtensionLib.sol";
 ...
 */
contract MerkleStorageInvalidator is
    ...
    ITakerInteraction
{
    ...
    function takerInteraction(
        ...
        bytes32 rootCalculated = takerData.proof.processProofCalldata(
            keccak256(
                abi.encodePacked(uint64(takerData.idx), takerData.secretHash)
            )
        );
        if (uint240(uint256(rootCalculated)) != rootShortened)
            revert InvalidProof();
        lastValidated[key] = ValidationData(
            takerData.idx + 1,
            takerData.secretHash
        );
    }
}
